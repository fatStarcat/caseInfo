      ...
      <!--<div id="messageSelect">-->
        ...
        <!--<div id="info-search">-->
          <!--<div class="select-item">-->
            ...
            <!--<DatePicker type="daterange" split-panels placeholder="请选择时间" :value="dateTime" style="width: 200px"></DatePicker>-->
          <!--</div>-->
          <!--<button class="search-btn btn-tabDefault">-->
            <!--查询-->
          <!--</button>-->
        <!--</div>-->
        <!--<hr/>-->
...
<script>
  export default {
      ...
      return {
        ...
        columns1: [//表头数据
          ...
          },
          {
            ...
            align: 'center',
            render: (h, params) => {
              if(params.row.status=='审核退回'||params.row.status=='审核通过已导出'||params.row.status=='审核通过待导出'){
                return h('div', [
                  h('span', {
                    props: {
                      size: 'small',
    ...
    created() {
      this.allData =  this.infoData = jsonData.docShield.examineData;
      this.total = this.infoData.length;
      this.loading = false;
      // let option = {
      //   type: 'get',
      ...
      //     _this.allData =  _this.infoData = val.examineData;
